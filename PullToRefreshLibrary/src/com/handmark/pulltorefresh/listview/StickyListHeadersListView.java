package com.handmark.pulltorefresh.listview;
...
public class StickyListHeadersListView extends ListView implements OnScrollListener {
    ...
    public interface OnHeaderClickListener2 {
        ...
        public void onHeaderClick(StickyListHeadersListView l, View header, int itemPosition,
                long headerId, boolean currentlySticky,float evX);
    }
    ...
    private OnHeaderClickListener2 onHeaderClickListener2;
    ...
    private StickyListHeadersAdapterWrapper.OnHeaderClickListener addapterHeaderClickListener =
            new StickyListHeadersAdapterWrapper.OnHeaderClickListener() {

                @Override
                public void onHeaderClick(View header, int itemPosition, long headerId) {
                    if (onHeaderClickListener != null) {
                        onHeaderClickListener.onHeaderClick(StickyListHeadersListView.this, header,
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        this.adapter =
                new StickyListHeadersAdapterWrapper(getContext(),
    ...
    private void scrollChanged(int firstVisibleItem) {
        ...
        if (currentHeaderId == null || currentHeaderId != newHeaderId) {
            ...
            header = adapter.getHeaderView(headerPosition, header, this);
            measureHeader();
        }
        ...
        if (childCount != 0) {
            ...
            if (viewToWatch != null
                ...
                if (firstVisibleItem == listViewHeaderCount
                        && super.getChildAt(0).getTop() > 0 && !clippingToPadding) {
                    headerBottomPosition = 0;
                } else {
                    if (clippingToPadding) {
                        headerBottomPosition =
                                Math.min(viewToWatch.getTop(), headerHeight + getPaddingTop());
                        headerBottomPosition =
                                headerBottomPosition < getPaddingTop() ? headerHeight
                                        + getPaddingTop() : headerBottomPosition;
                    } else {
                        headerBottomPosition = Math.min(viewToWatch.getTop(), headerHeight);
                        headerBottomPosition =
                                headerBottomPosition < 0 ? headerHeight : headerBottomPosition;
                    }
                }
            } else {
    ...
    public void setOnHeaderClickListener2(OnHeaderClickListener2 onHeaderClickListener) {
        this.onHeaderClickListener2 = onHeaderClickListener;
    }
            ...
            if (Math.abs(ev.getY() - headerDownY) < viewConfig.getScaledTouchSlop()) {
                if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
                    ...
                    if (onHeaderClickListener2 != null) {
                    	onHeaderClickListener2.onHeaderClick(this, header, headerPosition,
                                currentHeaderId, true,ev.getX());
                    }
                }
                return true;
            } else {
