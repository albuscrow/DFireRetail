 ...
 */
public class StoreReturnGoodsDetailItemActivity extends TitleActivity implements OnClickListener, IItemListListener, IOnItemSelectListener {
	...
	private void initData() {
		...
		searchGoodsVo = (SearchGoodsVo) map.get("returnGoodsReason");

		lblName.initData(searchGoodsVo.getGoodsName(), searchGoodsVo.getGoodsId());
		lblCode.initData(searchGoodsVo.getBarcode(), searchGoodsVo.getBarcode());
		lblPrice.initData(String.format("%f", searchGoodsVo.getPurchasePrice()), String.format("%f", searchGoodsVo.getPurchasePrice()));
	...
	private boolean valid() {
		if (StringUtils.isEmpty(txtNum.getStrVal())) {
			ToastUtil.showShortToast(StoreReturnGoodsDetailItemActivity.this, "退货数量为空！");
			return false;
		}
		return true;
	}
	...
	public void onItemListClick(ItemEditList obj) {
		if (obj.getId() == R.id.lsReason) {
			...
			// // 页面跳转从此添加 代码.
			Intent reason = new Intent(StoreReturnGoodsDetailItemActivity.this, StoreReturnGoodsReasonActivity.class);
	...
	private void save() {
		...
		Intent returngoods = new Intent(StoreReturnGoodsDetailItemActivity.this, StoreReturnGoodsAddActivity.class);
		...
		AsyncHttpPost httppost = new AsyncHttpPost(parameters, new RequestResultCallback() {
			...
			public void onSuccess(String str) {
				...
				if (dicVoList != null && dicVoList.size() > 0) {
					...
					shopsSpinner = new SpinerPopWindow(StoreReturnGoodsDetailItemActivity.this);
					shopsSpinner.refreshData(reasonStrList, 0);
					shopsSpinner.setItemListener(StoreReturnGoodsDetailItemActivity.this);
				}
